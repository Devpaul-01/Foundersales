...
- The service-role Supabase client bypasses Row-Level Security; the middleware chain (auth → workspace membership → rate limit) is the actual enforcement boundary, not RLS. That's a deliberate trade-off, documented plainly in [ARCHITECTURE.md](ARCHITECTURE.md) §9.1 rather than glossed over.
...
| Testing / CI | Unit tests, run automatically in CI on every push/PR |
...
- **Single-process deployment is still the documented default**, even though the split-process files exist in this repo — see [ARCHITECTURE.md](ARCHITECTURE.md) §11 for the reasoning behind the split and how to run it.
- **`pattern_insights`, a self-enqueued weekly job, currently has no registered handler** and fails on every run. This is a known, scoped gap — see [BACKGROUND_JOBS.md](BACKGROUND_JOBS.md) §4.3 for exactly what's ambiguous about the fix and why it wasn't guessed at.
...
- **Payments and subscriptions are not yet built.** There is no billing provider integration, no plan/tier enforcement, and no subscription state anywhere in the schema or API today — this is planned but still entirely unimplemented, not a partial or hidden feature.
